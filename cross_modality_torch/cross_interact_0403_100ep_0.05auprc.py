...
parser.add_argument('--l3', type=float, default=1000)
...
print(args)
weight_pth='crossInteraction_parallel_' + str(args.l0) + '_' + str(args.l1) + '_' + str(args.l2) + '_' + str(args.l3)
...
class net_crossInteraction(nn.Module):
    def __init__(self, lambda_l1, lambda_fused, lambda_group, lambda_bind):
        ...
        torch.nn.init.xavier_uniform_(self.aminoAcid_embedding.weight)
...
class crossInteraction(nn.Module):
    def __init__(self):
        ...
        #torch.nn.init.xavier_uniform_(self.crossInteraction0.weight)
        #torch.nn.init.xavier_uniform_(self.crossInteraction1.weight)
...
class net_comp_gcn(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.linear_final = nn.Linear(256, 256)
        torch.nn.init.xavier_uniform_(self.linear_final.weight)
...
if args.train == 1:
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for prot_data, drug_data_ver, drug_data_adj, prot_contacts, prot_inter, prot_inter_exist, label in train_loader:
            ...
            lossval=loss.detach().cpu().numpy()
            if epoch==-1 or batch%10==10:
                print('epoch', epoch, 'batch', batch,"/",batchsize,"=", lossval)
        ...
        if loss_epoch_val/batch_val < loss_val_best:
            ...
            torch.save(model.module.state_dict(), './weights/crossInteraction_parallel_' + str(args.l0) + '_' + str(args.l1) + '_' + str(args.l2) + '_' + str(args.l3) + '.pth')
...
model.load_state_dict(torch.load('./weights/crossInteraction_parallel_' + str(args.l0) + '_' + str(args.l1) + '_' + str(args.l2) + '_' + str(args.l3) + '.pth'))
model.eval()
