...
print(args)
weight_pth = 'concatenation_parallel_' + \
    str(args.l0) + '_' + str(args.l1) + '_' + str(args.l2) + '_' + str(args.l3)
...
class net_crossInteraction(nn.Module):
    def __init__(self, lambda_l1, lambda_fused, lambda_group, lambda_bind):
        ...
        self.joint_attn_prot, self.joint_attn_comp = nn.Linear(
            256, 256), nn.Linear(256, 256)
        self.tanh = nn.Tanh()
    ...
    def forward(self, prot_data, drug_data_ver, drug_data_adj,
                prot_contacts, prot_inter, prot_inter_exist, label):
        # protein embedding
        ...
        # TODO PROTEIN_3D_EMBEDDING=...
        # TODO CONCAT 3 WAY
        prot_embedding = self.crossInteraction(
            prot_seq_embedding, prot_graph_embedding)
        ...
        inter_comp_prot = self.sigmoid(
            torch.einsum(
                'bij,bkj->bik',
                self.joint_attn_prot(
                    self.relu(prot_embedding)),
                self.joint_attn_comp(
                    self.relu(comp_embedding))))
        inter_comp_prot_sum = torch.einsum('bij->b', inter_comp_prot)
        inter_comp_prot = torch.einsum(
            'bij,b->bij',
            inter_comp_prot,
            1 / inter_comp_prot_sum)
        ...
        cp_embedding = self.tanh(
            torch.einsum(
                ...
                prot_embedding,
                comp_embedding))
        cp_embedding = torch.einsum(
            'bijk,bij->bk', cp_embedding, inter_comp_prot)
        ...
        loss0, loss1, loss2 = self.loss_reg(
            inter_comp_prot, fused_matrix.to(
                inter_comp_prot.device), prot_contacts), self.loss_inter(
            inter_comp_prot, prot_inter, prot_inter_exist), self.loss_affn(
                affn_comp_prot, label)
        loss = loss0 + loss1 + loss2

        return loss

    def forward_inter_affn(self, prot_data, drug_data_ver,
    ...
    def loss_reg(self, inter, fused_matrix, prot_contacts):
        ...
        reg_loss = self.lambda_l1 * reg_l1 + self.lambda_fused * \
            reg_fused + self.lambda_group * reg_group
        return reg_loss

    def loss_inter(self, inter, prot_inter, prot_inter_exist):
        label = torch.einsum('b,bij->bij', prot_inter_exist, prot_inter)
        loss = torch.sqrt(((inter - label) ** 2).sum(dim=(1, 2))
                          ).mean() * self.lambda_bind
        return loss
...
class crossInteraction(nn.Module):
    def __init__(self):
        super().__init__()
        self.crossInteraction0 = nn.Sequential(
            nn.Linear(
                256, 256), nn.ReLU(), nn.Linear(
                256, 256))
        self.crossInteraction1 = nn.Sequential(
            nn.Linear(
                256, 256), nn.ReLU(), nn.Linear(
                256, 256))
        self.tanh = nn.Tanh()
...
class net_comp_gcn(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.ModuleList(
            [nn.Linear(43, 256), nn.Linear(256, 256), nn.Linear(256, 256)])
        self.relu = nn.ReLU()
        self.linear_final = nn.Linear(256, 256)
...
class dataset(torch.utils.data.Dataset):
    def __init__(self, name_split='train'):
        if name_split == 'train':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_train_data(
                args.data_processed_dir)
        elif name_split == 'val':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_val_data(
                args.data_processed_dir)
        elif name_split == 'test':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_test_data(
                args.data_processed_dir)
        elif name_split == 'unseen_prot':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_uniqProtein_data(
                args.data_processed_dir)
        elif name_split == 'unseen_comp':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_uniqCompound_data(
                args.data_processed_dir)
        elif name_split == 'unseen_both':
            self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, _, self.prot_inter, self.prot_inter_exist, self.label = load_uniqDouble_data(
                args.data_processed_dir)
        ...
        self.prot_data, self.drug_data_ver, self.drug_data_adj, self.prot_contacts, self.prot_inter, self.prot_inter_exist, self.label = torch.tensor(
            ...
            self.drug_data_ver).float().float(), torch.tensor(
    ...
    def __getitem__(self, index):
        return self.prot_data[index], self.drug_data_ver[index], self.drug_data_adj[index], self.prot_contacts[
            index], self.prot_inter[index], self.prot_inter_exist[index], self.label[index]
...
if args.train == 1:
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for prot_data, drug_data_ver, drug_data_adj, prot_contacts, prot_inter, prot_inter_exist, label in train_loader:
            prot_data, drug_data_ver, drug_data_adj, prot_contacts, prot_inter, prot_inter_exist, label = prot_data.cuda(
            ), drug_data_ver.cuda(), drug_data_adj.cuda(), prot_contacts.cuda(), prot_inter.cuda(), prot_inter_exist.cuda(), label.cuda()
        ...
        if loss_epoch_val / batch_val < loss_val_best:
            ...
            torch.save(model.module.state_dict(), './weights/concatenation_' +
                       str(args.l0) +
                       '_' +
                       str(args.l1) +
                       '_' +
...
model.load_state_dict(torch.load('./weights/concatenation_' +
                                 str(args.l0) +
                                 '_' +
                                 str(args.l1) +
                                 '_' +
...
with open('./logs/hpo_concatenation.log', 'a+') as f:
    f.write(str(args.l0) + ' ' + str(args.l1) + ' ' +
            str(args.l2) + ' ' + str(args.l3) + ' ')
eval_set = dataset('val')
eval_loader = torch.utils.data.DataLoader(
    dataset=eval_set,
    batch_size=args.batch_size,
    shuffle=False)
cal_affinity_torch(model, eval_loader, logging=True,
                   logpath='./logs/hpo_concatenation.log')
prot_length = np.load(data_processed_dir + 'prot_dev_length.npy')
comp_length = np.load(data_processed_dir + 'comp_dev_length.npy')
cal_interaction_torch(
    model,
    eval_loader,
    prot_length,
    comp_length,
