...
def Interaction_Rips(cut, protein_name, working_dir):
    ...
    print(
        "Computing 0-dimensional persistent homology with interaction distance."
    )

    com = np.load(working_dir + '/' + protein_name + '_' + str(cut) +
                  '.struct')
    PRO = com['PRO']
    LIG = com['LIG']
    for e1 in [['C'], ['N'], ['O'], ['S']]:
        for e2 in [['C'], ['N'], ['O'], ['S'], ['P'], ['F'], ['Cl'], ['Br'],
                   ['I']]:
            PROpts = []
            LIGpts = []
            for i in range(len(PRO)):
                p = str(PRO[i][0], encoding='ascii')
                if p.replace(" ", "") in e1:
                    PROpts.append(PRO[i][1])
            for i in range(len(LIG)):
                p = str(LIG[i][0], encoding='ascii')
                if p.replace(" ", "") in e2:
            ...
            if len(PROpts) > 0 and len(LIGpts) > 0:
                ...
                for ie in range(len(e2)):
                    namee2 = namee2 + e2[ie]
                Name = protein_name + '_' + namee1 + '_' + namee2 + '_' + str(
                    cut) + '.pts'
                OutFile = open(working_dir + '/' + Name, 'w')
                for i in range(len(PROpts)):
                    OutFile.write('1 ' + str(PROpts[i][0]) + ' ' +
                                  str(PROpts[i][1]) + ' ' + str(PROpts[i][2]) +
                                  '\n')
                for i in range(len(LIGpts)):
                    OutFile.write('0 ' + str(LIGpts[i][0]) + ' ' +
                                  str(LIGpts[i][1]) + ' ' + str(LIGpts[i][2]) +
                                  '\n')
                OutFile.close()
...
def Electrostatics_Rips(cut, protein_name, working_dir):
    ...
    print(
        "Computing 0-dimensional persistent homology with electrostatics interaction induced distance."
    )

    LIGELE = ['C', 'N', 'O', 'S', 'P', 'F', 'Cl', 'Br', 'I', 'H']
    PROELE = ['C', 'N', 'O', 'S', 'H']
    InFile = np.load(working_dir + '/' + protein_name + '_' + str(cut) +
                     '_chg.npz')
    LIG = InFile['LIG']
    PRO = InFile['PRO']
    for e1 in PROELE:
        for e2 in LIGELE:
            OutFile = open(
                working_dir + '/' + protein_name + '_' + e1 + '_' + e2 + '_' +
                str(cut) + '_chg.pts', 'w')
            for j in range(len(LIG)):
                if LIG[j][0] == e2:
                    OutFile.write('0 ' + str(LIG[j][1]) + ' ' +
                                  str(LIG[j][2]) + ' ' + str(LIG[j][3]) + ' ' +
                                  str(LIG[j][4]) + '\n')
            for j in range(len(PRO)):
                if PRO[j][0] == e1:
                    OutFile.write('1 ' + str(PRO[j][1]) + ' ' +
                                  str(PRO[j][2]) + ' ' + str(PRO[j][3]) + ' ' +
                                  str(PRO[j][4]) + '\n')
            OutFile.close()
...
def Alpha(cut, protein_name, working_dir):
    print(
        "Computing alpha complex based persistent homology for protein-ligand complex."
    )

    dt = np.dtype([('dim', int), ('birth', float), ('death', float)])
    ProEleCollection = [['C'], ['N'], ['O'], ['S'], ['C', 'N'], ['C', 'O'],
                        ['N', 'O'], ['C', 'N', 'O'], ['C', 'N', 'O', 'S']]
    LigEleCollection = [['C'], ['N'], ['O'], ['S'], ['C', 'N'], ['C', 'O'],
                        ['C', 'S'], ['N', 'O'], ['N', 'S'], ['O', 'S'],
                        ['C', 'N', 'O'], ['C', 'N', 'S'], ['C', 'O', 'S'],
                        ['N', 'O', 'S'], ['C', 'N', 'O', 'S'],
                        ['C', 'N', 'O', 'S', 'P', 'F', 'Cl', 'Br', 'I']]
            ...
            if len(propts) + len(ligpts) > 3:
                ...
                for pp in pt:
                    tmpoutfile.write(
                        ...
                        '\n')
                tmpoutfile.close()
                os.system('Rscript PH_Alpha.R ' + working_dir + '/pt.csv ' +
                          working_dir + '/tmp.out')
