...
if args.supervised == 0:
    ...
    model.load_state_dict(torch.load('../CPAC_pretrain/weights/concatenation_supervised_' + weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
...
if args.train == 1:
    ...
    for epoch in range(args.epoch):
        ...
        for prot_data, drug_data_ver, drug_data_adj, prot_contacts, prot_inter, prot_inter_exist, label in train_loader:
            ...
            print('epoch', epoch, 'batch', batch, lossv)
...
model = net_crossInteraction(args.l0, args.l1, args.l2, args.l3)
if args.supervised == 0:
    model.load_state_dict(torch.load('./weights/concatenation_' +  weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
else:
    model.load_state_dict(torch.load('./weights/concatenation_supervised_' +  weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
model.eval()
