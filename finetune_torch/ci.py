...
checkpoint_pth = "crossInteraction_"+weight_pretrain_pth + '_chechpoint.pth'
...
if args.supervised == 0:
    ...
    model.load_state_dict(torch.load('../CPAC_pretrain/weights/crossInteraction_supervised_' + weight_pretrain_pth + '.pth'))
model = model.cuda()
...
# evaluation
if args.supervised == 0:
    model.load_state_dict(torch.load('./weights/crossInteraction_' +  weight_pretrain_pth + '.pth'))
elif args.supervised == 1:
    torch.save(model.state_dict(), './weights/crossInteraction_supervised_' +  weight_pretrain_pth + '.pth')
model.eval()
