...
if args.supervised == 0:
    model.load_state_dict(torch.load('../pretrain_torch/weights/' + weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
elif args.supervised == 1:
    model.load_state_dict(torch.load('../CPAC_pretrain/weights/crossInteraction_supervised_' + weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
optimizer = torch.optim.Adam(model.parameters(), 1e-4)

fused_matrix = torch.tensor(np.load(args.data_processed_dir+'fused_matrix.npy'))
loss_val_best = 1e10
...
if args.train == 1:
    ...
    for epoch in range(args.epoch):
        ...
        for prot_data, drug_data_ver, drug_data_adj, prot_contacts, prot_inter, prot_inter_exist, label in train_loader:
            ...
            print('epoch', epoch, 'batch', batch,"/",len(train_loader) ,"=",loss0.detach().cpu().numpy(), loss1.detach().cpu().numpy(), loss2.detach().cpu().numpy())
        ...
        if loss_epoch_val/batch_val < loss_val_best:
            ...
            if args.supervised == 0:
                torch.save(model.state_dict(), './weights/crossInteraction_' +  weight_pretrain_pth + '.pth')
            elif args.supervised == 1:
                torch.save(model.state_dict(), './weights/crossInteraction_supervised_' +  weight_pretrain_pth + '.pth')
...
# evaluation
if args.supervised == 0:
    model.load_state_dict(torch.load('./weights/crossInteraction_' +  weight_pretrain_pth + '.pth',map_location=torch.device('cpu')))
elif args.supervised == 1:
    torch.save(model.state_dict(), './weights/crossInteraction_supervised_' +  weight_pretrain_pth + '.pth')
model.eval()
...
print('val')
with open('./logs/hpo.log', 'a+') as f:
    f.write(str(args.l0) + ' ' + str(args.l1) + ' ' + str(args.l2) + ' ' + str(args.l3) + ' ')
eval_set = dataset('val')
eval_loader = torch.utils.data.DataLoader(dataset=eval_set, batch_size=args.batch_size, shuffle=False)
cal_affinity_torch(model, eval_loader, logging=True, logpath='./logs/hpo.log')
prot_length = np.load(data_processed_dir+'prot_dev_length.npy')
comp_length = np.load(data_processed_dir+'comp_dev_length.npy')
cal_interaction_torch(model, eval_loader, prot_length, comp_length, logging=True, logpath='./logs/hpo.log')
with open('./logs/hpo.log', 'a+') as f:
    f.write('\n')
