...
def dssp_dict_from_pdb_file(in_file, DSSP="dssp"):
    try:
        return make_dssp_dict(in_file.replace(".pdb", ".dssp"))
    ...
    out_dict, keys = _make_dssp_dict(out)
    ...
    **Note** that DSSP can only handle one model.
    ...
    def __init__(self, model, pdb_file, dssp="mkdssp"):
        ...
        for key in dssp_keys:
            ...
            resname = PDBData.protein_letters_3to1.get(resname, 'X')
            if resname == "C":
