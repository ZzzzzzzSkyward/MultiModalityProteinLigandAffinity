...
parser.add_argument('--data_pretrain_dir', type=str, default='/scratch/user/yuning.you/dataset/pfam_dataset/')
...
parser.add_argument('--model', type=str, default='crossInteraction')
...
class dataset(torch.utils.data.Dataset):
    def __init__(self):
        ...
        self.label_clan, self.label_family = torch.tensor(np.load('/scratch/user/yuning.you/dataset/pfam_dataset/graph_clan.npy')), torch.tensor(np.load('/scratch/user/yuning.you/dataset/pfam_dataset/graph_family.npy'))
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        return self.prot_data[index], adj, self.prot_len[index], self.label_clan[index], self.label_family[index]
...
CI = True if args.model == 'crossInteraction' else False
model = net_pretrain_supervised(args.ss_seq, args.ss_graph, args.p_seq_mask, args.p_graph_mask, args.p_seq_replace, args.p_seq_sub, args.p_graph_drop, args.p_graph_sub, CI=CI)
...
if weight_pth == 'seq_none_graph_none':
    print('no self-supervised pretrain')
else:
    print('load weight ' + './weights/' + weight_pth + '.pth')
    model.module.model.load_state_dict(torch.load('./weights/' + weight_pth + '.pth'))
...
for epoch in tqdm(range(start_epoch, args.epoch)):
    ...
    for prot_data, prot_contacts, prot_len, label_clan, label_family in pretrain_loader:
        prot_data, prot_contacts, prot_len, label_clan, label_family = prot_data, prot_contacts, prot_len, label_clan, label_family

        optimizer.zero_grad()
        loss = model(prot_data, prot_contacts, prot_len, label_clan, label_family).mean()
    ...
    torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}, './weights/' + args.model + '_supervised_' + checkpoint_pth)
    if loss_epoch/batch < loss_epoch_best:
        print('save epoch ' + str(epoch) + ' weight')
        loss_epoch_best = loss_epoch/batch
        torch.save(model.module.model.state_dict(), './weights/' + args.model + '_supervised_' + weight_pth + '.pth')
